    ...
    <div class="min-h-screen py-5 mb-8">
        ...
        <div class="max-w-4xl lg:mx-auto sm:px-6 lg:px-8 mx-4">
            <div id="data-wrapper"
                ...
                @include('frontend.components.noti-paginate')
            </div>
...
<script>
    var ENDPOINT = "{{ route('notifications.index') }}";
    var page = 1;
    ...
    $(window).scroll(function() {
        if ($(window).scrollTop() + $(window).height() >= ($(document).height() - 20)) {
            page++;
            infiniteLoadMore(page); // Pass current filter to load more
        }
    });
    ...
    function infiniteLoadMore(page) {
        $.ajax({
                url: ENDPOINT + "?page=" + page , // Add filter to URL
                datatype: "html",
                type: "get",
                beforeSend: function() {
                    $('.auto-load').show();
